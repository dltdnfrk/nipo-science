...
Implemented and verified in `apps/local` are L05, L06, L07, L08, L09, L11, and the `ResearchIntent` half of L04. A loopback local interface now exists and serves the front end from its own origin, so LS01 has a listener to enforce against and does; L01 remains partial only in that a first-run installation flow is absent. Not implemented are L02, since Projects are an archival marker; L03; the ActionPlan half of L04; L10; and L12 as a reachable product surface. The Export contract exists in `packages/contracts` and `apps/local/nipo_local/exportpack.py` but has no route or screen.
