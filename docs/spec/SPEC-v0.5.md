...
Implemented and verified in `apps/local` are L02, L03, L04, L05, L06, L07, L08, L09, L10, L11, and L12. Local Projects and Sessions persist with list, resume, and archive that survive restart: creation through the local interface is readable after a full harness restart over the same data root, the session list is ordered by resume history, archiving a Project or Session blocks further writes with closed conflict codes, a duplicate archive is refused as a conflict rather than silently succeeding, and a Session link cannot be created for a Session that does not exist. The model turn is a reachable product surface: a Run-bound turn goes to exactly the provider and model the researcher selected, with no automatic fallback observable as `request_count` equal to one; the first successful turn pins that selection on the Run, a later turn naming anything else is refused before any provider contact, and every provider-neutral failure class (`authentication`, `rate_limit`, `quota`, `model_unavailable`, `provider_unavailable`, `timeout`, `transport`, and the rest) surfaces as the closed code `turn_failed` with no Artifact Version changed and no record persisted. The Run record keeps only the non-secret selection and turn digests; the assistant's text travels in the response and is not stored. Measurement-file loading is a reachable product surface: the approval screen loads a data file with its manifest sidecar through the local interface, the server parses it with the science loaders under the strict metadata policy, refusals carry the loader's and the science package's own issue tokens with zero durable records, and the accepted typed input starts the approved run. Run-start optionally pins the upload receipt's `input_sha256` — when the interface submits it, a document that no longer recomputes to that digest is refused with the closed code `input_digest_mismatch` before any durable read or write, and the approval stays unspent. `calibration_sha256` inside a manifest's `[calibration]` table is the researcher's claim about the calibration reference material — the product pattern-validates it and pins it into provenance but never recomputes it against the measurement bytes, which is the documented design, not an omission. The product path bounds uploads at 16 MiB per file, 250,000 image pixels, and 400,000 spectrum points; the science modules themselves keep their larger module-level limits. A loopback local interface now exists and serves the front end from its own origin, so LS01 has a listener to enforce against and does. The first-run flow is product-reachable: starting the local API creates the owner-only data root through `LocalPaths.ensure` before any token write — the token writer refuses to create the root itself, so a permissive umask can never widen the layout — and an empty workspace shows a guided first-run panel disclosing the data-root location outside synced directories, the loopback-only interface, and the in-process execution and Keychain residual risks of section 11. L01 remains partial only in that a packaged installer is out of scope for this repository. L04 is a reachable product surface for its ActionPlan half: the local interface creates an immutable plan from a complete `ResearchIntent`, displays the server-derived plan and intent digests, and grants the one-use approval, while editing any intent field invalidates the binding and demands a new plan. Run start is reachable through the local API with a typed scientific-input document, consuming the approval and the execution claim in one transaction. The Export Pack is a reachable product surface: the local interface plans pinned candidates, produces a pack into the owner-only export directory, lists and reclaims stored packs, and serves the bytes through a single-use download capability. AC-L12 is verified by named tests over the HTTP product path, and every AC-L12-B refusal is verified clause-for-clause at the exporting contract, whose path, link, collision, selection, and credential refusals the route inherits; the route itself additionally refuses non-pinned, duplicate, and empty selections and canonicalizes only the presentation order of the pinned set, which the manifest records.

Known divergences are enumerated in the manifest under `conformance.disclosed_divergences` and MUST remain disclosed until closed. They include in-process isolation, `skill_content_hashes` empty by construction, and a plaintext download signing key that section 7's prohibition covers only for provider credentials. The session-link execution reference divergence is closed: `session_links` records `producing_execution_id` and the ownership chain is test-backed.

Other divergences are also closed. The environment digest now covers every byte-shaping dependency section 6 requires: `workbench.environment_facts` pins `pillow`, `pydantic`, and `pydantic-core` alongside the interpreter, because `input_sha256` is taken over `ProbeInput.model_dump_json()` and the Markdown and ledger bytes derive from it. And the producing path now has a correction producer: `workbench.correct_analysis` re-runs an analysis against an existing Artifact and commits the next Version by compare-and-swap, leaving the Version it corrects byte-identical.
